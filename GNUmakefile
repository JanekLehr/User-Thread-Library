...
TARGETS := doTest doTest2 showHandler libULT.a alarmHelper parseUcontext myTests
...
handin.tar: clean
	tar cf handin.tar `find . -type f | grep -v '^\.*$$' | grep -v '/CVS/' | grep -v '/\.svn/' | grep -v '/\.git/' | grep -v 'lab[0-9].*\.tar\.gz' | grep -v '/\~/' | grep -v 'ULT.html' | grep -v '/\.tar/'` 
...
myTests.o: myTests.c interrupt.h ULT.h  $(MOREH)
	gcc -c $(CFLAGS) -o $@ myTests.c

myTests: myTests.o libULT.a 
	gcc $(CFLAGS) -o $@ myTests.o   $(ULT_LIB)
